...
if __name__ == '__main__':
    ...
    adata.obs['Treatment'].replace({'H': 'Hyperoxia',
                                    'N': 'Normoxia'},
                                   inplace=True
                                   )
    ...
    print(adata)
    adata.write(f'{output_data}/{adata_name}_cellranger_all_cells.gz.h5ad', compression='gzip')
    figures_qc = f'{figures}/qc'
    os.makedirs(figures_qc, exist_ok=True)
    sc.settings.figdir = figures_qc

    sc.pp.calculate_qc_metrics(adata, qc_vars = ['mt','ribo','hb'],expr_type='umis', percent_top=[20], log1p=True, inplace=True)

    sc.pl.scatter(adata, x='log1p_total_umis', y='log1p_n_genes_by_umis', show=False, save='_genes_by_counts_pretrim_log')
    sc.pl.highest_expr_genes(adata, n_top=20, show=False, save=f'_pretrim')
    sc.pl.violin(adata, ['log1p_total_umis'],groupby='Library',rotation=90,show=False, save = '_umis_pretrim')
    sc.pl.violin(adata, ['log1p_n_genes_by_umis'],groupby='Library', rotation=90,show=False,save = '_genes_pretrim')
    sc.pl.violin(adata, ['pct_umis_mt'],groupby='Library', rotation=90,show=False,save = '_mt_pretrim')
    ...
    sc.pl.scatter(adata, x='log1p_total_umis', y='log1p_n_genes_by_umis', show = False, save = '_genes_by_umis_posttrim_log')
    sc.pl.highest_expr_genes(adata, n_top=20, show = False, save = f'_posttrim')
    sc.pl.violin(adata, ['log1p_total_umis'],rotation=90,show=False,groupby='Library', save='_umis_posttrim')
    sc.pl.violin(adata, ['log1p_n_genes_by_umis'],rotation=90,show=False, groupby='Library',save='_genes_posttrim')
    sc.pl.violin(adata, ['pct_umis_mt'],groupby='Library', rotation=90,show=False,save = '_mt_posttrim')
    ...
    sc.pl.dotplot(adata,gene_dict['immune'],groupby='leiden',dendrogram=False,show=False,save='useful_genes.png')
    sc.tl.umap(adata,min_dist=0.3)
    for color in ['log1p_total_umis','log1p_n_genes_by_umis','Library','Treatment','Sex','leiden','doublet_score', 'predicted_doublet','celltype_rough']:
        ...
        for ind in range(0, num_obs):
            for subset in itertools.combinations(obs_list, ind):
                if len(subset) != 0:
                    ...
                    if len(subset) == 1:
                        key = subset[0]
                        adata.obs[key].value_counts().to_excel(
                            writer, sheet_name=key
                        )
                    else:
                        key = "_".join(subset)
                        adata.obs.groupby(subset[:-1])[
                            subset[-1]
                        ].value_counts(normalize=True).to_excel(writer, sheet_name=key[:31])
    adata.write(f'{output_data}/{adata_name}_filtered_embed.gz.h5ad', compression='gzip')
